...
Specific goals for the project didn't even solidify until after I'd started writing it.  Eventually I settled on
...
I had dream features as well - ones I think all content sites should have.  Interestingly, these features all come from MediaWiki.

An edit button on every page
------

I've been a Wikipedia editor for years now, and my first instinct when I see poor grammar or overly-long paragraphs is to click an "edit" button and fix the problem.  Most sites with typos don't have an edit button.

My posts may have typos, but I want anyone to be able to feel the catharsis of fixing someone else's mistakes on the internet.
...
This is, in my mind, MediaWiki/Wikipedia's killer feature.  Any page can be embedded in any other page - and since you can pass arbitrary parameters to the embedded document, you can use this single feature to build a domain-specific language for writing content.

I dug in to MediaWiki templates a bit when administrating the [MediaWiki documentation site for my previous employer](http://wikido.isoftdata.com), and I am totally sold on the feature.

Allowing embeddable templates would give me all that wiki power without the archaic MediaWiki formatting syntax.
...
It turns out that putting metadata in plain text isn't a new thing - Jekyll and others use YAML to achieve the same goal, and text-metadata-parser doesn't currently handle those three dashes/dots.  So, I'll likely be altering text-metadata-parser to support YAML at the top of Noddity content files at some point in the future.  C'est la vie.
...
Next I wrote [noddity-butler](https://github.com/TehShrike/noddity-butler) - which combines the noddity-retrieval blog structure with levelup-cache to return posts from a Noddity content directory (like [joshduff.com/content](http://joshduff.com/content/)) as efficiently as possible.

This library is used by both the client and the RSS/static servers to access a given site's content.

The actual front-end
--------

After assembling those parts on my nights and weekends, I made the real website.  Most of the front-end code by lines was taken up by the "recursively-embedded-posts" logic, which was not nearly as straightforward as it seemed in my head when I decided "hey, I should have embeddable templates!"
...
I didn't want to drop Wordpress and replace my real web site with my own creation until I had an RSS feed, and search engines could index my individual pages.
...
I registered [noddity.com](http://noddity.com) fairly early on.  I used the majority of a working-vacation with my wife at a bed-and-breakfast to write copy and fix bugs that I discovered in the process.

Cloning from github, running `npm install` + `npm build`, and deleting the leftover cruft is almost acceptable for just me, but it's nothing I'd tell my mom to do to set up her site.
...
Noddity is my most-starred project on Github, with [four stars](https://github.com/tehshrike/noddity) as I write this.  I don't know how to market it.  Given how much work the best open source developers seem to do to support their code, I'm not 100% sure I want to.
...
Playing rock shows in front of an audience is a million times more rewarding than playing for myself.

There's a special lift I feel when I get to act as a multiplier to someone else.  When I play with a band I can make some really sweet rock that makes people feel awesome.  When I write software I can give people a tool that they didn't have before.

So I want Noddity to seem usable.  Painless enough to override the "it would be easier to write it myself" instinct we all feel, and solid and featured enough that people would actually consider deploying it instead of Wordpress or Mediawiki.
...
There are other features I'd like - easier theming, easier plugins, better parsing of wiki-links in Markdown content (regular expressions are great, lexers are better).
...
But if Noddity grows, that would be sweet, and I'd love the excuse to put more effort into it.
